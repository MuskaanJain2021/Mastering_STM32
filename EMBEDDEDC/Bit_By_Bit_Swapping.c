...
uint8_t* reverseBits8(void* num) {
    uint8_t n = *((uint8_t*)num);
    uint8_t* reversed = (uint8_t*)malloc(sizeof(uint8_t));
    if (reversed != NULL) {
        *reversed = (uint8_t)reverseBitsHelper(n, 8);
    }
    return reversed;
}
...
uint16_t* reverseBits16(void* num) {
    uint16_t n = *((uint16_t*)num);
    uint16_t* reversed = (uint16_t*)malloc(sizeof(uint16_t));
    if (reversed != NULL) {
        *reversed = (uint16_t)reverseBitsHelper(n, 16);
    }
    return reversed;
}
...
uint32_t* reverseBits32(void* num) {
    uint32_t n = *((uint32_t*)num);
    uint32_t* reversed = (uint32_t*)malloc(sizeof(uint32_t));
    if (reversed != NULL) {
        *reversed = reverseBitsHelper(n, 32);
    }
    return reversed;
}
...
int main() {
    ...
    if (reversed32 != NULL) {
        ...
        free(reversed32); // Free dynamically allocated memory
    } else {
        printf("Failed to reverse bits for 32-bit number\n");
    }
    ...
    if (reversed16 != NULL) {
        ...
        free(reversed16); // Free dynamically allocated memory
    } else {
        printf("Failed to reverse bits for 16-bit number\n");
    }
    ...
    if (reversed8 != NULL) {
        typeHandlers[0].decToBin(reversed8, binStr); // 16-bit handler
        printf("Reversed value (8-bit) by bit by bit swapping technique in binary: %s\n", binStr);
        printf("Reversed value (8-bit) in hex: 0x%X\n", *reversed8);
        free(reversed8); // Free dynamically allocated memory
    } else {
        printf("Failed to reverse bits for 8-bit number\n");
    }
